...
from libs.common.make_metadata import make_dataframe
from libs.core.trainer import Trainer
from configs.setting import global_setting
from model.model_load import unet_load
...
def main():
    ...
    # 데이터 로더
    train_loader = build_dataloader(config=config, df=train_df, transform=transform.train, device=device, mode="train")
    valid_loader = build_dataloader(config=config, df=valid_df, transform=transform.valid, device=device, mode="valid")
    test_loader = build_dataloader(config=config, df=test_df, transform=transform.valid, device=device, mode='test')
